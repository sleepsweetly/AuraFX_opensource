...
interface Store3D {
  ...
  exportLayersToMainSystem: () => { layers: Layer[], elements: Element[] }
  importFromMainSystem: (elements: Element[], clearExisting?: boolean) => void
  importLayersFromMainSystem: (layers: Layer[], clearExisting?: boolean) => void
  ...
  persist(
    subscribeWithSelector(
      immer<Store3D>((set, get) => ({
        ...
        exportLayersToMainSystem: () => {
          const { vertices, shapes, layers } = get()
          ...
          const exportedLayers: Layer[] = []

          // Basit yaklaşım: Tüm vertex'leri export et
          if (vertices.size === 0) {
            console.log('No vertices to export!')
            return { layers: [], elements: [] }
          }

          console.log('Converting vertices to elements...')
          vertices.forEach((vertex, vertexId) => {
            console.log('Processing vertex:', vertexId, vertex)
            const element: Element = {
              ...
              color: vertex.color,
              particle: vertex.effectType || "particles"
            }
            allElements.push(element)
            console.log('Created element:', element)
          })

          // Tek bir layer oluştur
          if (allElements.length > 0) {
            ...
            exportedLayers.push(defaultLayer)
            console.log('Created layer with', allElements.length, 'elements')
          }

          console.log('=== EXPORT DEBUG END ===')
          console.log('Final result - Layers:', exportedLayers.length)
          console.log('Final result - Elements:', allElements.length)

          return { layers: exportedLayers, elements: allElements }
        },
        ...
        importFromMainSystem: (elements: Element[], clearExisting = false) => {
          ...
          const singleShape: Shape = {
            ...
            selected: false,
            particle: "particles",
            name: `Imported Drawing (${elements.length} elements)`,
        ...
        importLayersFromMainSystem: (layersToImport: Layer[], clearExisting = false) => {
          console.log('3D Store: Importing layers from main system:', layersToImport)

          if (layersToImport.length === 0) {
            console.log('No layers to import.')
            return
          }

          const scaleFactor = 0.4 // 2D elementleri 3D'de daha küçük yapmak için

          // Her katman için ayrı shape oluştur
          const newShapes: Shape[] = []
          const allNewVertices: Vertex[] = []
          const newLayers: Layer[] = []

          layersToImport.forEach((layer, layerIndex) => {
            if (layer.elements.length === 0) return

            // Layer'ı 3D için uyarla
            const adapted3DLayer: Layer = {
              ...layer,
              id: `imported_layer_${Date.now()}_${layerIndex}`,
              name: `${layer.name} (Imported)`,
              elements: [] // Elements will be recreated as vertices
            }
            ...
            // Bu katman için shape oluştur
            const shapeId = `imported_layer_${adapted3DLayer.id}_${Date.now()}`
            const layerShape: Shape = {
              id: shapeId,
              type: 'imported',
              position: { x: layerIndex * 2, y: 0, z: 0 }, // Katmanları yan yana yerleştir
              rotation: { x: 0, y: 0, z: 0 },
              scale: { x: 1, y: 1, z: 1 },
              vertices: [],
              visible: layer.visible !== false,
              selected: false,
              particle: layer.particle || "particles",
              name: `${layer.name} (${layer.elements.length} elements)`,
              elementCount: layer.elements.length,
              color: layer.color
            }

            // Bu katmanın vertex'lerini oluştur
            const layerVertices: Vertex[] = scaledElements.map((element, index) => {
              const vertexId = `vertex_${shapeId}_${index}`
              return {
                id: vertexId,
                position: {
                  x: (element.position.x - centerX) + layerShape.position.x,
                  y: (element.position.y - centerY) + layerShape.position.y,
                  z: (element.position.z - centerZ) + layerShape.position.z
                },
                effectType: element.particle || layer.particle || "particles",
                layer: adapted3DLayer.id,
                color: element.color || layer.color || "#ffffff",
                visible: layer.visible !== false,
                selected: false,
                groupId: shapeId
              }
            ...
            layerShape.vertices = layerVertices.map(v => v.id)
            newShapes.push(layerShape)
            allNewVertices.push(...layerVertices)

            console.log(`3D Store: Created shape for layer "${layer.name}" with ${layerVertices.length} vertices`)
          })

          console.log('3D Store: Created shapes:', newShapes)
          console.log('3D Store: Created vertices:', allNewVertices)
          console.log('3D Store: Created layers:', newLayers)

          set((state) => {
            if (clearExisting) {
              // Mevcut her şeyi temizle
              const newVertexEntries = allNewVertices.map(v => [v.id, v] as [string, Vertex])
              return {
                vertices: new Map(newVertexEntries),
                shapes: newShapes,
                layers: newLayers,
                vertexCount: allNewVertices.length
              }
            } else {
          ...
          console.log('3D Store: Successfully imported layers with shapes and vertices')
        ...
        debugStoreState: () => {
          const state = get()
          console.log('=== 3D STORE DEBUG ===')
          console.log('Vertices count:', state.vertices.size)
          console.log('Shapes count:', state.shapes.length)
          console.log('Layers count:', state.layers.length)
          console.log('Vertices:', Array.from(state.vertices.values()))
