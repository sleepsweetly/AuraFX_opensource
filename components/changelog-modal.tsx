      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md border border-gray-200/50 dark:border-zinc-800/80 rounded-2xl overflow-hidden shadow-2xl"
          ...
          >
            {/* Header */}
            <motion.div
              className="flex items-center justify-between p-6 border-b border-gray-200/50 dark:border-zinc-800/80"
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900 dark:text-zinc-100">What's New</h2>
                  <p className="text-gray-505 dark:text-zinc-400 text-sm">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={onClose}
                className="w-8 h-8 flex items-center justify-center text-gray-505 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-100 hover:bg-gray-100 dark:hover:bg-zinc-900 rounded-lg transition-colors"
              >
                <X className="w-4 h-4" />
              </motion.button>
            </motion.div>
            ...
            <div className="p-6">
              ...
              >
                <motion.div variants={itemVariants} className="mb-6">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-zinc-150">{currentEntry.title}</h3>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="space-y-3">
                  {currentEntry.changes.map((change, index) => (
                    <motion.div
                      ...
                      variants={itemVariants}
                      whileHover={{ x: 5 }}
                      className="flex items-start gap-3 p-3 bg-gray-50/70 dark:bg-zinc-900/40 rounded-lg border border-gray-100/50 dark:border-zinc-800/60 hover:bg-gray-100/70 dark:hover:bg-zinc-900/70 transition-colors"
                    >
                      <motion.div
                      ...
                      />
                      <span className="text-gray-700 dark:text-zinc-300 text-sm leading-relaxed">{change}</span>
                    </motion.div>
                  ))}
                </motion.div>
              </motion.div>
            ...
            {/* Navigation */}
            <motion.div
              className="flex items-center justify-between p-6 border-t border-gray-200/50 dark:border-zinc-800/80"
              variants={itemVariants}
            >
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={goPrev}
                className="flex items-center gap-2 px-4 py-2 text-gray-550 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-100 hover:bg-gray-100 dark:hover:bg-zinc-900 rounded-lg transition-colors"
              >
              ...
              <div className="flex gap-2">
                {CHANGELOG_DATA.map((_, index) => (
                  <motion.button
                    ...
                    className={`relative w-2 h-2 rounded-full transition-colors ${index === currentIndex ? 'bg-gray-900 dark:bg-zinc-100' : 'bg-gray-300 dark:bg-zinc-800'
                      }`}
                  ...
                  >
                    {index === currentIndex && (
                      <motion.div
                        className="absolute inset-0 bg-gray-900 dark:bg-zinc-100 rounded-full"
                        layoutId="activeDot"
