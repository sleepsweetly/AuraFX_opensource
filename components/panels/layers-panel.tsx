...
export function LayersPanel({
  ...
  return (
    <div
      ref={panelRef}
      className={`fixed left-0 top-0 h-full w-64 bg-white dark:bg-zinc-950 border-r border-gray-200 dark:border-zinc-850 shadow-xl z-50 transition-all duration-300 ease-out rounded-r-lg ${isOpen ? "translate-x-0 opacity-100" : "-translate-x-full opacity-0"
        }`}
    >
      <div className="flex flex-col h-full">
        ...
        <div className="border-b border-gray-200 dark:border-zinc-800">
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-3 hover:bg-gray-100 dark:hover:bg-zinc-900 transition-colors rounded-t-lg"
          >
            <div className="flex items-center gap-2">
              <Undo className="w-4 h-4 text-gray-700 dark:text-zinc-300" />
              <span className="font-medium text-sm text-gray-900 dark:text-zinc-100">History</span>
            </div>
            {historyExpanded ? <ChevronUp className="h-4 w-4 text-gray-700 dark:text-zinc-300" /> : <ChevronDown className="h-4 w-4 text-gray-700 dark:text-zinc-300" />}
          </button>
          {historyExpanded && (
            <div className="px-4 py-3">
              ...
              <div className="flex gap-2 mb-3">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                    canUndo() 
                      ? 'bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 border border-blue-200 dark:border-blue-800' 
                      : 'bg-gray-50 dark:bg-zinc-900/50 text-gray-400 dark:text-zinc-650 cursor-not-allowed border border-gray-200 dark:border-zinc-800'
                  }`}
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                    canRedo() 
                      ? 'bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/50 border border-green-200 dark:border-green-800' 
                      : 'bg-gray-50 dark:bg-zinc-900/50 text-gray-400 dark:text-zinc-650 cursor-not-allowed border border-gray-200 dark:border-zinc-800'
                  }`}
              ...
              {/* History Stats */}
              <div className="bg-gray-50 dark:bg-zinc-900/40 rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-gray-600 dark:text-zinc-400">Actions Available</span>
                  <span className="font-medium text-gray-900 dark:text-zinc-100">{past.length + future.length}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-gray-600 dark:text-zinc-400">Current Elements</span>
                ...
                {past.length > 0 && (
                  <div className="pt-2 border-t border-gray-200 dark:border-zinc-800">
                    <div className="text-xs text-gray-600 dark:text-zinc-400 mb-1">Last Action:</div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 hover:bg-gray-100 dark:hover:bg-zinc-900 transition-colors border-b border-gray-200 dark:border-zinc-800">
            <button
              onClick={() => setLayersExpanded(!layersExpanded)}
              className="flex items-center gap-2"
            >
              <Layers className="w-4 h-4 text-gray-700 dark:text-zinc-300" />
              <span className="font-medium text-sm text-gray-900 dark:text-zinc-100">Layers</span>
              {layersExpanded ? <ChevronUp className="h-4 w-4 text-gray-700 dark:text-zinc-300" /> : <ChevronDown className="h-4 w-4 text-gray-700 dark:text-zinc-300" />}
            </button>
            <button
              onClick={onAddLayer}
              className="w-8 h-8 hover:bg-gray-100 dark:hover:bg-zinc-900 text-gray-700 dark:text-zinc-300 rounded-lg flex items-center justify-center transition-colors"
            >
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hidden">
              {!displayLayers || displayLayers.length === 0 ? (
                <div className="px-4 py-8 text-center">
                  ...
                  <p className="text-sm text-gray-550">No layers yet</p>
                </div>
              ) : (
                displayLayers.map((layer, index) => (
                  <div
                    ...
                    className={`group px-4 py-2 hover:bg-gray-50 dark:hover:bg-zinc-900/40 cursor-pointer transition-colors border-b border-gray-100 dark:border-zinc-800/50 ${(displayCurrentLayer?.id === layer.id || currentLayerId === layer.id) ? 'bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800/50' : ''
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <Layers className={`w-4 h-4 ${(displayCurrentLayer?.id === layer.id || currentLayerId === layer.id)
                        ? 'text-blue-600'
                        : layer.visible !== false ? 'text-gray-700 dark:text-zinc-300' : 'text-gray-400 dark:text-zinc-550'
                        }`} />
                      ...
                      {editingLayerId === layer.id ? (
                        ...
                        <span className={`text-sm flex-1 ${(displayCurrentLayer?.id === layer.id || currentLayerId === layer.id)
                          ...
                          : layer.visible !== false ? 'text-gray-900 dark:text-zinc-100' : 'text-gray-400 dark:text-zinc-550'
                          }`}>
                          {layer.name}
                        </span>
                      )}
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        >
                          {layer.visible !== false ? (
                            <Eye className="w-4 h-4 text-gray-700 dark:text-zinc-300" />
                          ) : (
                            <EyeOff className="w-4 h-4 text-gray-400 dark:text-zinc-550" />
                          )}
                        </button>
                        <button
                          onClick={(e) => {
                          ...
                          title="Rename layer"
                        >
                          <Edit3 className="w-4 h-4 text-gray-700 dark:text-zinc-300" />
                        </button>
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                        ...
                        >
                          <Copy className="w-4 h-4 text-gray-700 dark:text-zinc-300" />
                        </button>
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
