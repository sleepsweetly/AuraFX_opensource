...
export function TopCenterToolbar({ viewMode = "top", setViewMode, modes, isRecording, onToggleRecording }: TopCenterToolbarProps) {
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40">
      <motion.div
        className="flex items-center gap-1 bg-white dark:bg-zinc-950 rounded-full px-3 py-2 shadow-lg border border-gray-200 dark:border-zinc-800"
        ...
        <DropdownMenu modal={false}>
          <DropdownMenuTrigger asChild>
            <motion.div whileHover="hover" whileTap="tap" variants={buttonVariants}>
              <Button variant="ghost" size="sm" className="h-8 gap-1 hover:bg-gray-100 dark:hover:bg-zinc-900 hover:text-gray-900 dark:hover:text-zinc-100 text-gray-700 dark:text-zinc-300">
                <motion.div
                  animate={{ rotate: 0 }}
          ...
          <DropdownMenuContent align="center" className="bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 z-50 shadow-lg" sideOffset={8} asChild>
              ...
              {Object.entries(VIEW_MODES).map(([mode, config]) => {
                ...
                return (
                  <motion.div key={mode} variants={dropdownItemVariants} whileHover={{ scale: 1.02, x: 5 }}>
                    <DropdownMenuItem
                      className={`text-black dark:text-zinc-300 flex items-center gap-2 ${isActive ? 'bg-gray-100 dark:bg-zinc-900' : ''} ${isDisabled ? 'opacity-50' : ''}`}
          ...
          {isRecButtonVisible && (
            ...
            >
              <motion.div
                ...
                className="h-4 w-px bg-gray-300 dark:bg-zinc-800 mx-1"
              ...
              <motion.div variants={recButtonVariants} whileHover="hover" whileTap="tap">
                <Button
                  ...
                  className={`h-8 gap-1 transition-colors ${isRecording ? 'text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700 dark:hover:text-red-400' : 'text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-900 hover:text-gray-900 dark:hover:text-zinc-100'}`}
                >
                  {isRecording ? (
                    <motion.div
